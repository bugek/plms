...
function LMSInitialize(dummyString) {
	...
	if ((flagInitialized) ) { return "false"; }
...
function LMSCommit(dummyString) {
	//alert("LMSCommit "+cache['cmi.core.score.raw']);
	...
	var params = 'SCOInstanceID=<?php print $SCOInstanceID; ?>&course_id='+course_id+'&item_id='+item_id+'&code='+d.getTime();
...
function LMSFinish(dummyString) {
	...
	// code to prevent caching
	var d = new Date();
	//alert("LMSFinish "+cache['cmi.core.score.raw']);
	delete cache['cmi.core.score.raw'];
	// set up request parameters - uses GET method
	req.open('GET','finish.php?SCOInstanceID=<?php print $SCOInstanceID; ?>&course_id=<?php print $course_id; ?>&code='+d.getTime(),false);
